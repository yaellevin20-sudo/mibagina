-- Fix get_my_playgrounds: remove conflicting DISTINCT ON that caused
-- "column ci.playground_id must appear in GROUP BY" (error 42803).
-- GROUP BY p.id already guarantees one row per playground; DISTINCT ON is redundant.
